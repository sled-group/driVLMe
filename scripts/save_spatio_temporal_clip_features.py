...
def load_video(vis_path, num_frm=15):
    vr = VideoReader(vis_path, ctx=cpu(0))
    total_frame_num = len(vr)
    total_num_frm = min(total_frame_num, num_frm)
    # frame_idx = get_seq_frames(total_frame_num, total_num_frm)
    frame_idx = list(range(total_frame_num-num_frm,total_frame_num))
...
def main():
    ...
    for video_name in tqdm(all_videos):
        ...
        if os.path.exists(os.path.join(clip_feat_path,video_id+".pkl")):  # Check if the file is already processed
            continue
        try:
            ...
            video_clip_features[video_id] = get_spatio_temporal_features(video_features.numpy().astype("float16"),num_temporal_tokens=15)
        ...
        if counter % 1 == 0:  # Save after every 512 videos, update this number as per your requirements
            for key in video_clip_features.keys():
                ...
                os.makedirs(os.path.dirname(os.path.join(clip_feat_path,key+".pkl")), exist_ok=True)
                with open(os.path.join(clip_feat_path,key+".pkl"), 'wb') as f:
