    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'video' in sources[0]:
            ...
            if isinstance(video_file,dict):
                features={}
                for view in video_file:
                    with open(f"{video_folder}/{video_file[view]}", "rb") as f:
                        features[view] = pickle.load(f)
                cur_token_len = 271  # 100 temporal + 256 spatial, TODO: Hard Coding is not good
                ...
                # print(sources)
                # print([e["conversations"] for e in sources])
                sources = preprocess_multimodal(
                    copy.deepcopy([e["conversations"] for e in sources]),
                    self.multimodal_cfg, cur_token_len)
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if 'video' in instances[0]:
            if isinstance(instances[0]["video"],dict):
                features={}
                for view in instances[0]["video"]:
                    features[view] = [torch.tensor(instance['video'][view]) for instance in instances]
                    if all(x is not None and x.shape == features[view][0].shape for x in features[view]):
                        features[view] = torch.stack(features[view])
                    else:
