...
from drivlme.model import VideoChatGPTLlamaForCausalLM
...
def initialize_model(model_name, projection_path=None,lora_path = None,device="cuda:0"):
    """
    ...
    if not lora_path:
        ...
        lora_cfg_pretrained = AutoConfig.from_pretrained(lora_path)
        ...
        if os.path.exists(os.path.join(lora_path, 'non_lora_trainables.bin')):
            ...
            non_lora_trainables = load_from_hf(lora_path, 'non_lora_trainables.bin')
        # print([k for k,v in non_lora_trainables.items()])
        ...
        # Load image processor
        image_processor = CLIPImageProcessor.from_pretrained(model.config.mm_vision_tower, torch_dtype=torch.float16)
        print(model.lm_head.weight.shape[0])
