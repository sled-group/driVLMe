...
class VideoChatGPTLlamaModel(LlamaModel):
    ...
    def forward(
        ...
        if (input_ids.shape[1] != 1 or self.training) and video_spatio_temporal_features is not None:
            if isinstance(video_spatio_temporal_features,dict):
                video_features={}
                for view in video_spatio_temporal_features:
                    video_features[view] = self.mm_projector(video_spatio_temporal_features[view])
                ...
                for cur_input_ids, cur_input_embeds in zip(input_ids, inputs_embeds):
                    if self.vision_config.use_vid_start_end:
                        ...
                        views=["CAM_FRONT","CAM_FRONT_RIGHT","CAM_FRONT_LEFT","CAM_BACK","CAM_BACK_RIGHT","CAM_BACK_LEFT"]
                        for i,video_start_token_pos in enumerate(video_start_tokens):
                            cur_video_features = video_features[views[i]][cur_video_idx].to(device=cur_input_embeds.device)
